...
class Request extends _body.default {
  constructor(input, init = {}) {
    ...
    const inputBody = init.body ? init.body : isRequest(input) && input.body !== null ? (0, _body.clone)(input) : null;
    super(inputBody, {
      size: init.size || input.size || 0
    });
    const headers = new _headers(init.headers || input.headers || {});

    if (inputBody !== null && !headers.has('Content-Type')) {
      const contentType = (0, _body.extractContentType)(inputBody, this);
...
const getNodeRequestOptions = request => {
  ...
  const headers = new _headers(request[INTERNALS].headers); // Fetch step 1.3
  ...
  if (request.body !== null) {
    const totalBytes = (0, _body.getTotalBytes)(request); // Set Content-Length if totalBytes is a number (that is not NaN)
  ...
  const search = (0, _getSearch.getSearch)(parsedURL); // Pass the full URL directly to request(), but overwrite the following
  // options:
